...
Parameter uncertainty is estimated from the asymptotic covariance matrix of the converged fit, `cov = (RSS / dof) * inv(J'J)`, where `J` is the Jacobian at the optimum and `dof` is the residual degrees of freedom. Standard errors are the square roots of its diagonal, and the 95% confidence interval for each `log10 K` is the estimate plus or minus the Student-t quantile for that `dof` times its standard error. Intervals are computed in `log10 K` and converted to `K` for display, so they are asymmetric about `K`; the standard error quoted for `K` uses the delta method, `SE(K) = K * ln(10) * SE(log10 K)`. Parameter correlations come from the same covariance matrix. This is the standard large-sample approximation for nonlinear least squares and assumes the model is locally linear near the optimum, which the rank, conditioning and sensitivity gates above already enforce for any reported fit. On the bundled synthetic designs, a Monte-Carlo check of the nominal 95% interval measured 0.95-0.97 coverage for 1:1 and 0.94-0.98 for 1:2 and 2:1.
