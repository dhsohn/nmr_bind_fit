...
`nmr_bind_fit` [@nmrbindfit2026] is a Python command-line workflow for reproducible NMR chemical-shift titration analysis. It fits 1:1, sequential 1:2, sequential 2:1, and non-binding candidate models under a fast-exchange assumption, ranks the fitted candidates using information criteria, estimates parameter uncertainty from the asymptotic covariance matrix of each converged fit, and writes a self-contained HTML report with the fitted values, model comparison, diagnostics, and plots. The software is intended as a transparent decision-support tool: it reports the best-supported working model among tested candidates, while making clear that final chemical interpretation must also consider spectral behavior, saturation, peak assignment, and feasible stoichiometry.
...
`nmr_bind_fit` occupies a deliberately narrower position within this software landscape. It is designed specifically for host-resonance NMR chemical-shift titrations and applies the same fixed, inspectable candidate set to each dataset: 1:1 binding, sequential 1:2 binding, sequential 2:1 binding, and a non-binding linear-drift control. The workflow compares these candidates using a standardized information-criterion table and produces a self-contained HTML report containing fitted values, uncertainty estimates, diagnostics, warnings, and plots. The explicit non-binding candidate is included to expose datasets for which a systematic chemical-shift trend can be fitted without invoking a binding equilibrium.

The contribution of `nmr_bind_fit` is therefore not a new general-purpose equilibrium solver or a replacement for tools that support arbitrary chemical models. Instead, it provides a reproducible command-line decision-support workflow for a constrained but common NMR use case. Its fixed candidate set, negative control, provisional model language, and report generation are intended to make the assumptions and limitations of an analysis easier to inspect, communicate, and reproduce.
